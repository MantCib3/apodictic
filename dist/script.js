...
async function init() {
    ...
    if (path.match(/^\/article\/([^/]+)/) && articleDetail) {
        ...
        shareButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                const shareTitle = button.getAttribute('data-share-title') || 'Article';
                if (navigator.share) {
                    navigator.share({
                        title: shareTitle,
                        url: shareUrl
                    ...
                    alert(`Sharing is not supported in this browser. Please copy the URL to share: ${shareUrl}`);
                }
            });
        });
        return;
    ...
    document.addEventListener('click', (e) => {
        ...
        const articleCard = e.target.closest('.section-article, .event, .finance-event, .related-article-card');
        if (articleCard) {
